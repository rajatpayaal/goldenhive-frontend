...
export function ChatbotWidget({ title = "Help Center", isOpen: controlledOpen, onClose: controlledOnClose }) {
  ...
  // ── Controlled / sheet mode (used by MobileBottomNav) ──────────────────────
  if (isControlled) {
    if (!open) return null;
    return (
      <div className="gh-surface flex max-h-[85vh] w-full flex-col overflow-hidden rounded-t-[30px] border-t border-[color:var(--gh-border)] bg-[color:var(--gh-surface-strong)] shadow-[0_-8px_40px_rgba(0,0,0,0.18)]">
          ...
          <div className="mt-4 grid gap-3">
            <div className="rounded-[26px] border border-[color:var(--gh-border)] bg-[linear-gradient(135deg,rgba(255,255,255,0.92),rgba(255,242,231,0.88))] p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {["Booking confirmation", "Cancel booking", "Refund policy", "Contact support"].map((text) => (
                  <button key={text} type="button" onClick={() => setQuery(text)}
            ...
            <div className="grid gap-3">
              ...
              {faqs.length === 0 && status !== "loading" ? (
                <div className="rounded-[24px] border border-[color:var(--gh-border)] bg-white px-4 py-4 text-sm font-semibold text-[color:var(--gh-text-soft)]">No matching FAQs found.</div>
              ) : null}
              {faqs.length > 0 ? (
                <div className="grid gap-2">
                  {faqs.slice(0, 6).map((item) => (
                    <button key={item?._id || item?.question} type="button" onClick={() => setSelected(item)}
  ...
  // ── Standalone floating mode (web/desktop) ─────────────────────────────────
  return (
